...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	void moveItem(int from,int to) {
		Decreaser<T> temp=null;
		temp = this.array[from];
	...
	private void heapify(int where) {
		...
		if(rightchild<=size) {
		if((array[where].getValue()).compareTo(array[leftchild].getValue())<=0 && (array[where].getValue()).compareTo(array[rightchild].getValue())<=0) {
			return;
		}
		if((array[rightchild].getValue()).compareTo(array[leftchild].getValue())<=0){
			moveItem(where,rightchild);
			heapify(rightchild);
		}else{
			moveItem(where,leftchild);
			heapify(leftchild);
		}
		}else if(leftchild<=size) {
			 if((array[where].getValue()).compareTo(array[leftchild].getValue())>0) {
				 moveItem(where,leftchild);
				 heapify(leftchild);
			 }
		}
