...
public class AVLTree<T extends Comparable<T>> {
	...
	private void updateHeight(TreeNode<T> root) {
		...
		root.height=Math.max(rightheight, leftheight)+1;
	}
	...
	private TreeNode<T> rightRotate(TreeNode<T> root) {
		...
		TreeNode<T> left_child = root.left;
		TreeNode<T> left_right_child = left_child.right;
		left_child.parent = root.parent;
		left_child.setRight(root);	
		root.setLeft(left_right_child);
		updateHeight(root);
	...
	private TreeNode<T> leftRotate(TreeNode<T> root) {
	    // FIXME: fill in the rotation code
		TreeNode<T> right_child = root.right;
		TreeNode<T> right_left_child = right_child.left;
		right_child.parent = root.parent;
		right_child.setLeft(root);	
		root.setRight(right_left_child);
		updateHeight(root);
