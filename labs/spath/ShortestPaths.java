...
public class ShortestPaths {
    ...
    public void run() {
    	...
    	//
    	int pathWeight;
    	VertexAndDist minVertex ; 
    	while(!pq.isEmpty()) {
    		minVertex = pq.peek(1);
    		Decreaser<VertexAndDist> minvd = handles.get(minVertex.vertex);
    		pathWeight = minvd.getValue().distance;
    		for (Edge edge : minVertex.vertex.edgesFrom()) {
    			int newDist = pathWeight + this.weights.get(edge);
    			int oldDist = handles.get(edge.to).getValue().distance;
    			if(newDist < oldDist) {
    				VertexAndDist newVertex = new VertexAndDist(edge.to, newDist);
    				handles.get(edge.to).decrease(newVertex);
    				parentEdges.put(edge.to, edge);
    			}
        	}
    		pq.extractMin();
    	}
